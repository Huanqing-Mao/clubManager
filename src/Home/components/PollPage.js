...
function PollPage() {
    ...
    const choosePoll = useCallback((num) => {
        setChoice(num);
    }, [choice]);
                ...
                <ul>
                    {polls.map(item => (
                        ...
                        hoverable={true}>
                            ...
                            <font color="red">{moment().isAfter(moment(item.deadline)) ? "Closed" : ""}</font>
                        </Card></li>
                    ))}
                </ul>  
            </div>
            <div className="poll_form">
                <PollQuestions pollID={choice} cID={currentID} deletePoll={deletePoll}/>
            </div>
            <div className="create_poll">
            <Popover title="Create new poll" trigger="click" content={content} open={open} onOpenChange={handleOpenChange}>
