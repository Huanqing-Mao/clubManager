...
function PollQuestions( { pollID, cID, deletePoll } ) {
    ...
    const [message, setMessage] = useState("You have not submitted your vote yet, please vote before the deadline!")
    ...
    async function getVoteRecord() {
        ...
        if (vr !== null && vr[0] !== undefined) {
            ...
            setVoted("Update");
            setMessage("We have already recorded your response, you could still update your vote before the deadline!")
        }
    ...
    async function formStatus() {
        if (passedDdl) {
            setForm(<Button type="primary" htmlType="submit" onClick={()=>updateVote()} className="AntButton">
            {voted}
        </Button>);
        } else {
